...
class AGCRNCell(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim):
        ...
        self.gate = AVWGCN(
            dim_in + self.hidden_dim, 2 * dim_out, cheb_k, embed_dim
        )
        self.update = AVWGCN(
            dim_in + self.hidden_dim, dim_out, cheb_k, embed_dim
        )
...
class AVWDCRNN(nn.Module):
    def __init__(
        self, node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers=1
    ):
...
class AGCRN(nn.Module):
    ...
    def forward(self, source, placeholder):
        # source: B, T_1, N, D
        # target: B, T_2, N, D
        ...
        init_state = self.encoder.init_hidden(source.shape[0], self.num_node)
        ...
        if self.use_norm:
            ...
            output = output * (stdev[:, 0, :].unsqueeze(1).repeat(1, self.horizon, 1))
            output = output + (means[:, 0, :].unsqueeze(1).repeat(1, self.horizon, 1))
