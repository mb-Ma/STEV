...
class Mask_STDataset(Dataset):
    ...
    def __init__(
        self, path, normalizer=None, incre_num=1, is_col=False
    ) -> None:
        self.path = path
    ...
    def get_MeanStd(self, mean, std):
        scaler = StandardScaler(mean, std, self.is_col)
        return scaler
...
class STDataset(Dataset):
    ...
    def __init__(
        self, path, types, normalizer=None, norma_name=None, is_col=False
    ) -> None:
        self.is_col = is_col
        ...
        self.x, self.y = data
        self.x, self.y, self.ts = (
            self.x[..., :-1].astype(np.float32),
            self.y[..., :-1].astype(np.float32),
            self.x[..., 0],
        )

        if self.ts is None:
            print(f"{types}: X shape: {self.x.shape}, Y shape: {self.y.shape}")
        else:
            print(
...
def get_dataloaders(cfg):
    ...
    if cfg.is_oracle:
        train = STDataset(
            cfg.data_path, "train", norma_name=cfg.scaler, is_col=cfg.is_col
        )
    elif cfg.incre_num == 1:
        train = STDataset(
            cfg.data_path,
            "train" + str(cfg.incre_num),
            norma_name=cfg.scaler,
            is_col=cfg.is_col,
        )
    elif cfg.is_last:
        train = STDataset(
        ...
        train = Mask_STDataset(
            cfg.data_path,
            normalizer=cfg.scaler,
            incre_num=cfg.incre_num,
            is_col=cfg.is_col,
        )
    scaler = train.scaler
    ...
    valid = STDataset(
        cfg.data_path, "valid", normalizer=scaler, is_col=cfg.is_col
    )
    ...
    test = STDataset(
        cfg.data_path, "test", normalizer=scaler, is_col=cfg.is_col
    )
