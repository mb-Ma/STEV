...
class MaskStat:
    ...
    def compute(self):
        feats = [(0, self.n_feats[0])] + [
            (self.n_feats[i], self.n_feats[i + 1])
            for i in range(len(self.n_feats) - 1)
        ]
        stat = {
            "mean": [],
            "std": [],
...
def generate_all_data(data, ts_data, len_x, len_y, rules, is_col=False):
    ...
    for i, (n_feats, n_day) in enumerate(rules):
        n_idx = n_day * 24 # hourly sampling rate.
...
def main(data_path, path_out, len_x, len_y, expanding_rules, is_col):
   # Loading data, 2017-01-01 00：00：00  2018-12-30 23：00：00
    df = pd.read_csv(data_path)
    df.set_index("Unnamed: 0", inplace=True)
    data = df['2017-01-01 00:00:00':].values
    ts_feat = df.index[-data.shape[0]:]
    data = data.astype(np.float32)  # [N, num_feature]
...
if __name__ == "__main__":
    path = Path("../raw/")
    data_path = path / "weather_temp.csv"
    len_x, len_y = 12, 12
    path_out = Path("../processed/weather-560_310-7_410-3_410-12_12.npz")
    # 取2年数据， 2017-01-01 00：00：00  2018-12-30 23：00：00 sum up to 729 days
    expanding_rules = [(310, 560), (410, 7), (410, 3), (410, 159)]
    is_col = False # if column-wise normalize
